...
Java_now_link_mastigias_data_taglib_TagLibBridge_nativeReadMetadata(
    ...
    try {
        TagLib::FileStream stream(filePath.c_str(), true);
        if (!stream.isOpen()) {
            LOGE("nativeReadMetadata: Failed to open file stream read-only: %s", filePath.c_str());
            return nullptr;
        }
        ...
        if (f.isNull() || !f.file()) {
            LOGE("nativeReadMetadata: Failed to parse audio file: %s", filePath.c_str());
            return nullptr;
        }
        ...
        LOGE("nativeReadMetadata: Exception reading metadata for %s: %s", filePath.c_str(), e.what());
        return nullptr;
    } catch (...) {
        LOGE("nativeReadMetadata: Unknown exception reading metadata for %s", filePath.c_str());
        return nullptr;
    }
}
...
Java_now_link_mastigias_data_taglib_TagLibBridge_nativeReadArtwork(
    ...
    try {
        TagLib::FileStream stream(filePath.c_str(), true);
        if (!stream.isOpen()) {
            LOGE("nativeReadArtwork: Failed to open file stream read-only: %s", filePath.c_str());
            return nullptr;
        }
        ...
        if (f.isNull() || !f.file()) {
            LOGE("nativeReadArtwork: Failed to parse file: %s", filePath.c_str());
            return nullptr;
        }

        TagLib::ByteVector artworkData;
        ...
        if (auto* flacFile = dynamic_cast<TagLib::FLAC::File*>(file)) {
            ...
            for (auto* pic : pictures) {
                if (pic) {
                    ...
                    if (!bv.isEmpty()) {
                        if (pic->type() == TagLib::FLAC::Picture::FrontCover || artworkData.isEmpty()) {
                            artworkData = bv;
        ...
        // 2. MPEG (MP3)
            ...
            if (mpegFile->ID3v2Tag()) {
                ...
                for (auto* frame : frames) {
                    ...
                    if (picFrame) {
                        TagLib::ByteVector bv = picFrame->picture();
                        if (!bv.isEmpty()) {
                            if (picFrame->type() == TagLib::ID3v2::AttachedPictureFrame::FrontCover || artworkData.isEmpty()) {
                                artworkData = bv;
        ...
        // 3. MP4 / M4A / AAC
            ...
            if (mp4File->tag()) {
                ...
                if (items.contains("covr")) {
                    ...
                    if (!covers.isEmpty()) {
                        TagLib::ByteVector bv = covers.front().data();
                        if (!bv.isEmpty()) {
                            artworkData = bv;
                        }
                    }
        ...
        if (artworkData.isEmpty() || artworkData.size() == 0) {
            return nullptr;
        }

        jbyteArray jResult = env->NewByteArray(static_cast<jsize>(artworkData.size()));
        if (!jResult) return nullptr;

        env->SetByteArrayRegion(
            jResult, 0, static_cast<jsize>(artworkData.size()), reinterpret_cast<const jbyte*>(artworkData.data()));
        return jResult;
    } catch (const std::exception& e) {
        LOGE("nativeReadArtwork: Exception reading artwork for %s: %s", filePath.c_str(), e.what());
        return nullptr;
    } catch (...) {
        LOGE("nativeReadArtwork: Unknown exception reading artwork for %s", filePath.c_str());
        return nullptr;
    }
}
...
Java_now_link_mastigias_data_taglib_TagLibBridge_nativeWriteMetadata(
    ...
    try {
        ...
        LOGE("nativeWriteMetadata: Exception writing metadata for %s: %s", filePath.c_str(), e.what());
        return JNI_FALSE;
    } catch (...) {
        LOGE("nativeWriteMetadata: Unknown exception writing metadata for %s", filePath.c_str());
        return JNI_FALSE;
    }
}
